...
public class SqlAnaly {
    ...
    private static List<String> analyColumnLine(String sql) {
    ...
    private static String analyTableName(String sql) {
    ...
    private static String analyPrimaryKey(String sql) {
    ...
    public static void main2(String[] args) throws Exception {
        String s = "CREATE TABLE `transaction` (  `ID` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '序号',   `app` varchar(20) NOT NULL COMMENT '应用',   `model` varchar(20) NOT NULL COMMENT '应用内的模块',   `key` varchar(50) NOT NULL COMMENT '事务自定义标记',   " +
                "`resume_bean_name` varchar(50) NOT NULL COMMENT '事务补偿当前对象',   `resume_method` varchar(255) NOT NULL COMMENT '事务补偿方法',   `resume_parameter` longtext NOT NULL COMMENT '事务方法参数',   `exception` varchar(50) NOT NULL COMMENT '异常标记串',   " +
                "`exception_info` varchar(255) NOT NULL COMMENT '异常信息',   `exception_stack` longtext NOT NULL COMMENT '异常堆栈',   `step` tinyint(4) NOT NULL COMMENT '事务进度',   `status` tinyint(4) NOT NULL COMMENT '事务状态',   `mark` varchar(32) NOT NULL COMMENT" +
                " '当前事务标记,为各种信息md5结果',   `gmt_create` datetime NOT NULL COMMENT '创建时间',   `gmt_modified` datetime NOT NULL COMMENT '修改时间',   `is_deleted` tinyint(4) NOT NULL COMMENT '记录是否逻辑删除',   PRIMARY KEY (`id`),   UNIQUE KEY `mark_2` (`mark`),   KEY " +
                "`mark` (`mark`),   KEY `app` (`app`,`model`,`key`,`mark`),   KEY `key` (`key`) ) ENGINE=InnoDB  DEFAULT CHARSET=latin1 COMMENT='事务记录表' AUTO_INCREMENT=2 ;";
        SqlInfo si = analy(s);
    ...
    public static void main12(String[] args) throws Exception {
        String s = "create table dis_push_item_detail (id bigint unsigned not null comment '主键',     gmt_create datetime not null comment '创建时间',     gmt_modified datetime not null comment '修改时间',     mark_id bigint unsigned not null comment '商品Id" +
                "(长度18位内，以99开头的纯数字)',     topic_title varchar(255) not null comment '专题标题',     push_time datetime not null comment '投放时间(yyyy-mm-dd hh：mm：ss)',     url varchar(512) not null comment '主题详情链接地址',     img varchar(512) not null comment " +
                "'主题图片地址',     user_img varchar(512) not null comment '所属人图片地址',     shop_title varchar(128) not null comment '店铺名称',     user_id bigint unsigned not null comment '商家id',     shop_type varchar(50) not null comment '店铺类型',     shop_url " +
                "varchar(512) not null comment '店铺地址',     shop_logo varchar(512) not null comment '店铺logo',     shop_desc varchar(256) comment '店铺描述',     topic_id bigint unsigned not null comment '投放主题id',     primary key (id),     unique key mark_id " +
                "(mark_id)   ) comment='发现主题商品推送详情';";
        SqlInfo si = analy(s);
        List<Column> list = si.getColumnList();
        List<Column[]> l = new ArrayList<>();
        for (Column c : list) {
